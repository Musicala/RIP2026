...
(function () {
  ...
  async function loadOfficialStudentsIndex() {
    ...
    // Fuente preferida: directorio local de identidades rip-musicala/students,
    // sincronizado desde estudiantes-musicala por la Cloud Function
    // syncStudentIdentity. El id devuelto es el studentId CANONICO.
    try {
      const localStudents = await window.RIPRepository.loadStudents();
      const byEmail = new Map();
      let docsRead = 0;
      for (const raw of localStudents || []) {
        const emails = (Array.isArray(raw.emails) ? raw.emails : (raw.email ? [raw.email] : []))
          .map(normalizeEmail)
          .filter(e => e.includes('@'));
        if (!emails.length) continue;
        const canonicalId = String(raw.studentId || raw.officialStudentId || '').trim() || String(raw.id || '').trim();
        const student = {
          id: canonicalId,
      ...
      if (byEmail.size) {
        ...
        officialStudentsReadStats = { collections: 1, docs: docsRead, emails: byEmail.size };
        officialStudentsError = '';
        console.info('Identidades locales (rip/students) leidas:', officialStudentsReadStats);
        return;
      }
      console.info('Directorio local sin correos todavia; usando fallback estudiantes-musicala.');
    } catch (err) {
      console.warn('No se pudo leer el directorio local de identidades:', err);
    }

    // FALLBACK TEMPORAL: lectura directa de estudiantes-musicala desde el
    // navegador. Se elimina al terminar el backfill de identidades hacia
    // rip-musicala (y quedara bloqueada por las nuevas reglas del Formulario).
  ...
  async function upload() {
    ...
    try {
      ...
      for (const saved of result.saved) {
        const classKey = duplicateClassKey(saved);
