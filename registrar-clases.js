...
(function () {
  ...
  async function loadOfficialStudentsIndex() {
    setStatus('Leyendo el directorio canónico de estudiantes...');
    try {
      ...
      console.info('Directorio canónico local leído:', officialStudentsReadStats);
    } catch (err) {
      console.error(err);
      officialStudentsError = err?.message || 'No se pudo cargar el directorio canónico local.';
    }
  }
  ...
  async function saveLocalEmailLink(row) {
    ...
    if (!email || !name || !canonical) {
      throw new Error('No se puede guardar la relación Wix sin un studentId canónico verificado.');
    }
    try {
      const env = await window.RIPFirebase.ready;
  ...
  function validate(normalized) {
    ...
    if (!normalized.studentId) issues.push('sin studentId canónico');
