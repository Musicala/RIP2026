...
(function () {
  ...
  function ensureServicesDatalist() {
    let dl = document.getElementById('ripDefaultServicesList');
    if (!dl) {
      dl = document.createElement('datalist');
      dl.id = 'ripDefaultServicesList';
      document.body.appendChild(dl);
    }
    const services = window.RIPRepository?.getDefaultServices?.() || [];
    dl.innerHTML = services
      .map(s => `<option value="${escapeHTML(s.name || s)}"></option>`)
      .join('');
    return dl;
  }
  ...
  function buildFieldsHTML(rowData) {
    return FIELDS.map((f) => {
      ...
      const listAttr = f.key === 'servicio' ? ' list="ripDefaultServicesList"' : '';
      return `<label class="ripedit-field"><span class="ripedit-label">${escapeHTML(f.label)}</span><input type="${f.type === 'date' ? 'date' : 'text'}" id="ripedit_${f.key}" class="control" value="${escapeHTML(val)}"${listAttr}></label>`;
    }).join('');
  }
