...
(function () {
  ...
  // Completa y valida row.studentId contra el directorio local sincronizado.
  // Un ID con apariencia válida pero ausente del índice también se rechaza:
  // el cliente nunca crea identidades ni acepta identificadores alternativos.
  async function attachStudentId(row) {
    if (!row) return row;
    const id = identity();
    if (!id) {
      throw new Error('No está disponible el resolutor de identidad y RIP exige studentId canónico.');
    }

    const explicitStudentId = String(row.studentId || '').trim();
    if (explicitStudentId) {
      const index = await id.ensureIndex().catch(() => null);
      if (!index?.byCanonicalId?.has(explicitStudentId)) {
        throw new Error('El studentId indicado no existe en el directorio canónico sincronizado.');
      }
      row.studentId = explicitStudentId;
      row.studentIdSource = row.studentIdSource || 'explicit';
      return row;
    }
    let resolved = null;
    try {
    ...
    throw new Error(
      `No se encontró studentId canónico para "${row.estudiante}". ` +
      'Resuelve la identidad mediante el flujo administrativo antes de guardar.'
    );
  }
  ...
  /*
    `students` es una réplica de identidad administrada por backend. El
    navegador solo confirma que el studentId existe; nunca crea, completa ni
    marca documentos canónicos o heredados.
  */
  async function upsertStudent(env, row) {
    const canonical = String(row.studentId || '').trim();
    if (!canonical) {
      throw new Error('RIP exige studentId canónico antes de guardar datos operativos.');
    }
    const { doc, getDoc } = env.fs;
    const snapshot = await getDoc(doc(env.db, 'students', canonical));
    if (!snapshot.exists()) {
      throw new Error('El studentId no está disponible en el directorio canónico sincronizado.');
    }
    const identityData = snapshot.data() || {};
    const storedId = String(identityData.studentId || identityData.officialStudentId || snapshot.id).trim();
    if (storedId !== canonical || snapshot.id !== canonical) {
      throw new Error('El directorio contiene una identidad inconsistente; no se realizó ninguna escritura.');
    }
  }
  ...
  */
  async function recalculateStudent(studentId) {
    ...
    // Resolver contra el índice: nunca se publica un resumen bajo una llave
    // por nombre ni bajo un ID que no exista en el directorio canónico.
    let canonical = '';
    let nameKey = isCanonicalId(inputKey) ? '' : inputKey;
    try {
      const index = await identity()?.ensureIndex();
      const direct = index?.byCanonicalId?.get(inputKey);
      if (direct) {
        canonical = inputKey;
        nameKey = direct.nameKey || '';
      } else {
    ...
    } catch (_err) { /* el error explícito se emite abajo */ }
    if (!canonical) {
      throw new Error('No se puede recalcular estado sin studentId canónico verificado.');
    }
  ...
  async function addRegistroRow(data) {
    ...
    clearCache('registro');
    await recalculateStudent(row.studentId);
  ...
  async function addClienteB2C(data) {
    ...
    for (const user of clean) {
      if (!user.estudiante) throw new Error('Cada pago debe identificar al estudiante.');
      const helper = {
      ...
      await attachStudentId(helper);
      user.studentId = helper.studentId;
    }
    const row = {
      fecha,
      fechaTs: calc.parseDate(fecha)?.getTime() || 0,
  ...
  async function updateClienteB2C(recordId, data) {
    ...
    for (const u of clean) {
      if (!u.estudiante) throw new Error('Cada pago debe identificar al estudiante.');
      const helper = {
        estudiante: u.estudiante,
        estudianteKey: calc.norm(u.estudiante),
        studentId: u.studentId
  ...
  async function deleteRegistroRow(recordId) {
    ...
    if (before) {
      const identityRow = normalizeRegistro(before);
      await attachStudentId(identityRow);
      canonicalStudentId = identityRow.studentId;
    }
    await deleteDoc(ref);
    clearCache('registro');
    if (canonicalStudentId) await recalculateStudent(canonicalStudentId);
  ...
  */
  async function resolveScheduleDoc(env, studentIdOrName) {
    ...
    if (isCanonicalId(text)) {
      try {
        const index = await identity()?.ensureIndex();
        const entry = index?.byCanonicalId?.get(text);
        if (entry) {
          canonical = text;
          nameKey = entry.nameKey || '';
          displayName = entry.name || text;
        }
      } catch (_err) { /* sin índice: no se acepta el identificador */ }
    } else {
      nameKey = C().norm(text);
      try {
  ...
  async function saveSchedule(studentId, fechas) {
    ...
    // Toda programación nueva va exclusivamente al documento canónico. Los
    // documentos heredados por nombre siguen disponibles solo para lectura.
    const key = resolved.canonical;
    if (!key) throw new Error('No se puede guardar programación sin studentId canónico verificado.');
  ...
  */
  ...
  async function mergeStudents(sourceStudentIdOrName, targetStudentIdOrName, targetDisplayName, options = {}) {
    throw new Error(
      'La fusión de identidades está deshabilitada en el navegador. ' +
      'Usa el flujo administrativo backend con snapshot y validación.'
    );

    /* istanbul ignore next -- defensa adicional; las Rules también niegan students writes */
    const plan = await buildMergePlan(sourceStudentIdOrName, targetStudentIdOrName, targetDisplayName);
